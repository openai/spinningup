...
class ContinuousSacAgent:
    def __init__(
        ...
        # This is the entropy regularisation coefficient
        self.alpha = alpha
    ...
    def test(self, env):
        for j in range(self.num_test_episodes):
            ...
            while not done:
                ...
                # only render the environment on the first time
                # if j == 0:
                #     env.render()
                ...
                state = next_state
            if j == 0:
                # close the environment if you rendered it
                env.close()
            self.logger.store(TestEpRet=episode_return, TestEpLen=episode_length)

    def train(self, train_env, test_env):
        ...
        for t in range(total_steps):
            ...
            if (t + 1) % self.steps_per_epoch == 0:
                epoch_number = (t + 1) // self.steps_per_epoch
...
class DiscreteSacAgent(ContinuousSacAgent):
...
if __name__ == '__main__':
    ...
    agent = DiscreteSacAgent(train_env.observation_space, train_env.action_space)
